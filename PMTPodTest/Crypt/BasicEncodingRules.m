...
- (id)visitBERInteriorNode:(id)node
{
    ...
    for (NSUInteger i = 0; i < count; i++) 
    {
        ...
        if (i < count - 1) [self.string appendString:middleDelimiter]; // Scott change from appendFormat to appendString August 28, 2014 3:49 PM
    }
    [self decreaseIndent];
    [self.string appendFormat:@" )"];
...
- (NSData*)berData
{
    NSMutableData *berData = [[[NSMutableData  alloc] init] autorelease];
...
- (NSData*)berHeader
{
    NSMutableData *berHeader = [[[NSMutableData  alloc] init] autorelease];
...
{
    NSMutableData *lengthStorageData = [[[NSMutableData alloc] init] autorelease];
...
- (NSData*)zeroByteData
{
    ...
    NSMutableData *zeroByteData = [[[NSMutableData  alloc] init] autorelease];
...
- (void)beMutable
{
    NSMutableArray *newArray = [[[NSMutableArray alloc] initWithCapacity:[self.collection count]] autorelease];
...
- (NSData*)berContents
{
    NSMutableData *berContents = [[[NSMutableData  alloc] init] autorelease];
    for (NSUInteger i = 0; i < [self.collection count]; i++) {
...
{
    BerTaggedCollection *newSet = [[[BerTaggedCollection alloc] init] autorelease];
...
{
    BerTaggedData *newData = [[[BerTaggedData alloc] init] autorelease];
...
{
    BerTaggedString *newString = [[[BerTaggedString alloc] init] autorelease];
